import sys
sys.path.append(r'D://PyCharmCode//nlp_nnlm//nnlm')
    ...
    with graph.as_default():
        ...
        with tf.variable_scope('nnlm' + 'weight'):
            ...
            softmax_u = tf.Variable(tf.truncated_normal([args.hidden_num , args.vocab_size],
                            stddev=1.0 / math.sqrt(args.hidden_num)))
            d=tf.Variable(tf.truncated_normal([args.hidden_num]))
            b=tf.Variable(tf.truncated_normal([args.vocab_size]))

        #构造计算图
        def infer_output(input_data):
            # step 1: hidden = tanh(x * H + d)
            # step 2: outputs = softmax(x * W + hidden * U + b)
            input_x=tf.reshape(tf.nn.embedding_lookup(embeddings,input_data),[-1,args.win_size*args.word_dim])
            hidden=tf.tanh(tf.matmul(input_x,weight_h)+d)
            hidden_output=tf.matmul(input_x,softmax_w)+tf.matmul(hidden,softmax_u+b)
        ...
        optimizer = tf.train.AdagradOptimizer(0.09).minimize(loss)
...
if __name__ == '__main__':
    main()
    # input_data = tf.placeholder(tf.int32, [120, 5])
    # print(input_data)
    # targets = tf.placeholder(tf.int64, [120, 1])
    # print(targets)
