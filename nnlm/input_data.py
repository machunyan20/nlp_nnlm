...
#         for sent in sentences:
#             word_counts.update(sent)
#         print(word_counts)#可以看作是整篇文档的词频
#         vocabulary_inv = ['<START>', '<UNK>', '<END>'] + \
#                          [x[0] for x in word_counts.most_common() if x[1] >= 2]
#         vocabulary = {x: i for i, x in enumerate(vocabulary_inv)}
#
#         print(vocabulary_inv)#['<START>', '<UNK>', '<END>', ',', 'and', 'the', 'of', 'its', '.']
#         print(vocabulary)
        #{'<START>': 0, '<UNK>': 1, '<END>': 2, ',': 3, 'and': 4, 'the': 5, 'of': 6, 'its': 7, '.': 8}